# -*- coding: utf-8 -*-
"""
Created on Tue Apr 28 12:16:02 2026
...
# =============================================================================
# BLOQUE 1: CARGA DE DATOS DIGITALIZADOS (REUNIÓN, densidad = 2.5)
# =============================================================================
print("1. Cargando datos digitalizados...")

ruta_csv = r"C:\Users\Usuario\Gravity-data-\VOLCANES\Reunion\Puntos_Reunion_1_2.5_Completos.csv"
...
rho_roca = 2500  # Densidad estándar
rho_agua = 1030
...
fig_1 = pygmt.Figure()
fig_1.basemap(region=region, projection="M15c", frame=["af", 'WSen+t"Reunion {rho} = 2.5 g/cm3: Bouguer Simple (Digitalizado)"'])
fig_1.coast(shorelines="0.5p,black", water="lightblue", land="lightgray", resolution="f")

pygmt.makecpt(cmap="turbo", series=[a_bouguer_simple.min(), a_bouguer_simple.max()])
fig_1.plot(x=lon, y=lat, style="c0.3c", fill=a_bouguer_simple, cmap=True, pen="0.1p,black")
fig_1.colorbar(frame='af+l"mGal"', position="JBC+w10c+h")
fig_1.show()

fig_2 = pygmt.Figure()
fig_2.basemap(region=region, projection="M15c", frame=["af", 'WSen+t"Reunion {rho} = 2.5 g/cm3: Bouguer Completa (Digitalizado)"'])
fig_2.coast(shorelines="0.5p,black", water="lightblue", land="lightgray", resolution="f")

pygmt.makecpt(cmap="turbo", series=[a_bouguer_completa.min(), a_bouguer_completa.max()])
fig_2.plot(x=lon, y=lat, style="c0.3c", fill=a_bouguer_completa, cmap=True, pen="0.1p,black")
...
df_final.to_csv("Reunion_2_5_Resultados_Completos.csv", index=False)
