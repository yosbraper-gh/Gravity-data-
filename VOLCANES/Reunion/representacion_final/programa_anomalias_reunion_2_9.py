# -*- coding: utf-8 -*-
"""
Created on Tue Apr 28 12:58:36 2026
...
# =============================================================================
# BLOQUE 1: CARGA DE DATOS DIGITALIZADOS (REUNIÓN, densidad = 2.9)
# =============================================================================
print("1. Cargando datos digitalizados...")

ruta_csv = r"C:\Users\Usuario\Gravity-data-\VOLCANES\Reunion\Puntos_Reunion_3_Completos_AB_2_9.csv"
...
a_bouguer_simple = df['A (mGal)'].values

# Definimos la región de Reunion y la proyección UTM 4N
region = [55.15, 55.90, -21.45, -20.80]
proyeccion = pyproj.Proj(proj="utm", zone=40, ellps="WGS84")
...
rho_roca = 2900  # Densidad estándar
rho_agua = 1030
...
fig_1 = pygmt.Figure()
fig_1.basemap(region=region, projection="M15c", frame=["af", 'WSen+t"Reunion {rho} = 2.9 g/cm3: Bouguer Simple (Digitalizado)"'])
fig_1.coast(shorelines="0.1p,black", water="lightblue", land="lightgray", resolution="f")

pygmt.makecpt(cmap="turbo", series=[a_bouguer_simple.min(), a_bouguer_simple.max()])
fig_1.plot(x=lon, y=lat, style="c0.2c", fill=a_bouguer_simple, cmap=True, pen="0.1p,black")
fig_1.colorbar(frame='af+l"mGal"', position="JBC+w10c+h")
fig_1.show()

fig_2 = pygmt.Figure()
fig_2.basemap(region=region, projection="M15c", frame=["af", 'WSen+t"Reunion {rho} = 2.9 g/cm3: Bouguer Completa (Digitalizado)"'])
fig_2.coast(shorelines="0.1p,black", water="lightblue", land="lightgray", resolution="f")

pygmt.makecpt(cmap="turbo", series=[a_bouguer_completa.min(), a_bouguer_completa.max()])
fig_2.plot(x=lon, y=lat, style="c0.2c", fill=a_bouguer_completa, cmap=True, pen="0.1p,black")
fig_2.colorbar(frame='af+l"mGal"', position="JBC+w10c+h")
fig_2.show()
...
df_final.to_csv("Reunion_2_9_Resultados_Completos.csv", index=False)
