# -*- coding: utf-8 -*-
"""
Created on Thu Apr 16 13:34:14 2026

@author: Usuario
"""

"""
PRIMER PROGRAMA: REPRESENTACIÓN DE ANOMALÍAS OBENIDAS DE LOS MAPAS
"""
...
# 1. CARGAR DATOS
# ============================================================
df = pd.read_csv("datos_completos_faial_2.csv")

lon = df["Longitud"].values
lon = lon /1000 #En el csv aparecen los valores sin comas
lat = df["Latitud"].values
lat = lat /1000
anomalia = df["AB (mGal)"].values
...
# trabaje con distancias reales y no con grados, es decir, se trabaja en UTM

# Coordenadas entrada; Coordenadas de la rejilla; Fuerza a Python a leer 
#siempre los datos como (Longitud, Latitud)
transformer = Transformer.from_crs("EPSG:4326", "EPSG:32626", always_xy=True)

# Esto hace lo contrario: se usa  para que, después de calcular todo en metros, 
# el mapa nos muestre coordenadas que entendamos (grados).
transformer_inv = Transformer.from_crs("EPSG:32626", "EPSG:4326", always_xy=True)
...
# ============================================================
# En lugar de rellenar huecos entre los puntos que se obtuvieron del mapa
# esta librería crea masas ficticias, a una profundidad determinada, para generar los 
# mGals que se midieron en la superficie.
...
bbox = box(-28.880600,35.506505,-28.566198,38.669620)
tierra_faial = tierra_union.intersection(bbox)
...
dentro = np.array([
    tierra_faial.contains(Point(lo, la))
...
ax.set_extent([-28.880600,38.506505,-28.566198,38.669620], crs=ccrs.PlateCarree())
...
ax.set_title(
    "Complete Bouguer Anomaly Map\nFaial (mGal) — Harmonica Equivalent Sources",
    fontsize=12, fontweight="bold"
)

plt.tight_layout()
plt.savefig("mapa_bouguer_faial_final.png", dpi=200, bbox_inches="tight")
plt.show()