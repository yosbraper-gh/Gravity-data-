# -*- coding: utf-8 -*-
"""
Created on Mon Apr 27 12:46:13 2026
...
# =============================================================================
# BLOQUE 1: CARGA Y TRANSFORMACIÓN DE DATOS (GRAN CANARIA UTM)
# =============================================================================
print("1. Cargando datos de Gran Canaria y transformando coordenadas...")

ruta_csv = r"C:\Users\Usuario\Gravity-data-\VOLCANES\Canarias\Gran_Canaria\2022_GranCanaria_Bouguer_Gravity_FGMontesinos_et_al.dat"

# Leemos el archivo delimitado por espacios
df = pd.read_csv(ruta_csv, sep='\s+', skiprows=2, 
                 names=['X_utm', 'Y_utm', 'H', 'Gravedad_mGal', 'Bouguer_Original'])

# --- LA SOLUCIÓN AL ERROR ESTÁ AQUÍ ---
# Forzamos a Pandas a convertir TODAS las columnas en números matemáticos (float/int).
# Si hay alguna letra o símbolo raro por error en el archivo, lo convierte en NaN (vacío).
df = df.apply(pd.to_numeric, errors='coerce')

# Borramos cualquier fila que tenga vacíos o errores
df = df.dropna()

# FILTRO 1: Valores absurdos
df = df[(df['Gravedad_mGal'] > 970000) & (df['Gravedad_mGal'] < 990000)]
df = df[(df['H'] > -50) & (df['H'] < 2000)] # El Pico de las Nieves mide ~1956m

# --- EL PASO CLAVE: DE METROS (UTM) A GRADOS (LAT/LON) ---
proyeccion = pyproj.Proj(proj="utm", zone=28, ellps="WGS84")
# Usamos inverse=True para pasar de X/Y a Lon/Lat
lon_dd, lat_dd = proyeccion(df['X_utm'].values, df['Y_utm'].values, inverse=True)

# Asignamos a los arrays base
lon   = lon_dd
lat   = lat_dd
h     = df['H'].values
g_obs = df['Gravedad_mGal'].values

print(f" -> Se han cargado {len(g_obs)} puntos correctamente.")

# =============================================================================
# BLOQUE 2: GRAVEDAD NORMAL Y AIRE LIBRE
# =============================================================================
# Definimos la región para Gran Canaria
region = [-15.85, -15.35, 27.70, 28.20]
...
densidades_completas = [2400, 2500, 2600, 2700] 
resultados_bc_multi = {}
...
nombre_archivo = r"C:\Users\Usuario\Gravity-data-\VOLCANES\Canarias\Gran_Canaria\Datos_post_programacion_Gran_Canaria.csv"
